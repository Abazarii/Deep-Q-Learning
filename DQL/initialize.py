...
class make_sol(object):
    ...
    def calc_Cmax(self):
        ...
        self.makespan = evaluations.calculate_completion_times(sequence_to_cython, self.processing_times,
                                                               self.machines_number)
        return self.makespan

    def calculate_idle_times(self):
        """Calculate the idle time wrt each job and saves in self.idle_time."""
        sequence_to_cython = np.array(self.sequence, dtype='int32')
        memory_view_object = evaluations.idle_times_calc(sequence_to_cython, self.processing_times,
                                                         self.machines_number)
    ...
    def insert_in_best_position(self, job, tie_breaking=False):
        ...
        best_position, self.makespan, best_positions, best_makespans = evaluations.taillard_acceleration(
            sequence_to_cython, self.processing_times, job, self.machines_number, use_tie_breaking)
...
def NEH_heuristic(sol_init, local_optimum=True, tie_breaking_within_NEH=False, tie_breaking_partial_initial=False,
                  LS_within_NEH='nothing'):
    """
    --> Generate initial sol_init with NEH heuristic
    --> reference: Nawaz, Enscore and Hans heuristic (1983)
    :type sol_init: object
    """
    # Order jobs
...
def extract_taillard(inst_number):
    ...
    directory = "C:/Users/safir/Desktop/PFSP/instance sets/taillard instances/"
...
def extract_vrf(inst_number, size):
    if size == 'small':
        ...
        directory = "C:/Users/safir/Desktop/PFSP/instance sets/vrf instances/Small/"
        instances_names = list()
    ...
    directory = "C:/Users/safir/Desktop/PFSP/instance sets/vrf instances/Large/"
    instances_names = list()
